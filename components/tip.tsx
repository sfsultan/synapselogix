...
export function Tip({ title, intro, description, hasMore }: TipProps) {
  ...
  return (
    <Collapsible
      ...
      className="my-10 pt-5 pb-7 space-y-2 border border-red-400 dark:border-red-900 rounded relative bg-zinc-100 dark:bg-zinc-800 shadow-lg"
    >
      <div className="flex items-center justify-between space-x-4 px-5 border rounded absolute py-2 -top-5 left-5 bg-red-500">
        <h4 className="text-md font-semibold text-white">{title}</h4>
      </div>
      <div className="flex items-center justify-center rounded-full absolute py-2 h-10 w-10 -top-7 right-10 bg-red-500 text-white">
